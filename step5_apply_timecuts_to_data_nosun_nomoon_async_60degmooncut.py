...
def worker(i, timesteps_start_removed, timesteps_end_removed, file_to_add, filetype, npz_array_location):
	while True:
		...
		if len(job) < 2: ##These will be the -1 lines
			...
			runnum=0
			if(filetype == '.npz'):
				savearray = np.zeros((fs.yaml_ebins, int(180.0/fs.yaml_binsz), int(360.0/fs.yaml_binsz))) ##make sure this exists to send back
...
if RANK >= 0: ##Only need to do this on the main node
	...
	files_to_add = 'data_nosun_nomoon/data' ##There are four things to add, so we want to run 5 cores
	filetypes = '.npz'
	npz_array_locations = 'arr_0'
