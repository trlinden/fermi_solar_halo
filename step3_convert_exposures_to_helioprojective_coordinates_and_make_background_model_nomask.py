...
def worker(i, in_pixels, in_Tyvals, in_Txvals):
    while True:
        ...
        if( len(job) < 2):
            break
        elif os.path.isfile('solar_exposure_nomask/solar_exposure.' + str(job[0]) + '.' + str(job[1]) + '.npz') or os.path.isfile('exposures_sourcecuts/nexp2.' + str(job[0]) + '.' + str(job[1]) + '.fits') == 0: ##This will be the -1 lines
            if(os.path.isfile('solar_exposure_nomask/solar_exposure.' + str(job[0]) + '.' + str(job[1]) + '.npz')):
                print('File: solar_exposure/solar_exposure.' + str(job[0]) + '.' + str(job[1]) + '.npz was found') 
            if(os.path.isfile('exposures/nexp2.' + str(job[0]) + '.' + str(job[1]) + '.fits') == 0):
                print('File: exposures/nexp2.' + str(job[0]) + '.' + str(job[1]) + '.fits was not found')
            ...
            hdulist = fits.open('exposures/nexp2.' + str(starttime) + '.' + str(endtime) + '.fits')
            ...
            fits.writeto('exposures_nosun_nomoon_nomask/nexp2.' + str(starttime) + '.' + str(endtime) + '.fits', scidata)
            ...
            np.savez_compressed('solar_exposure_nomask/solar_exposure.' + str(starttime) + '.' + str(endtime) + '.npz', a=sub_exposure_array)
            hdulist.close()
            COMM.send(0, dest=0, tag=12)
...
if RANK == 0: ##Only need to do this on the main node
    ...
    os.system('mkdir ' + str(fs.yaml_wkdir) + '/solar_exposure_nomask')
    os.system('mkdir ' + str(fs.yaml_wkdir) + '/exposures_nosun_nomoon_nomask')
