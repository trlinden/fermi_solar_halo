...
def worker(i, timesteps_start_removed, timesteps_end_removed, file_to_add, filetype, npz_array_location):
	while True:
		...
		if len(job) < 2: ##These will be the -1 lines
			...
			runnum=0
			if(filetype == '.npz'):
				savearray = np.zeros((fs.yaml_ebins, fs.yaml_healpix_numpixels)) ##make sure this exists to send back
				for x in range(startline, endline):
					fileexists=1
					try:
						openfile = np.load(file_to_add + '.' + str(timesteps_start_removed[x]) + '.' + str(timesteps_end_removed[x]) + '.npz')
						print("The file:" + file_to_add + '.' + str(timesteps_start_removed[x]) + '.' + str(timesteps_end_removed[x]) + '.npz' + " was loaded")
					except:
						fileexists=0
						pass
					if(fileexists):
						array = openfile[npz_array_location]
						if(runnum==0): ##For the first one
							savearray = array
							runnum += 1
						else:
...
if RANK >= 0: ##Only need to do this on the main node
	...
	files_to_add = 'solar_exposure_nomask/solar_exposure' ##There are four things to add, so we want to run 5 cores
	filetypes = '.npz'
	npz_array_locations = 'a'
	...
	outputfile = open('write_output_file_for_solar_exposure_nosun_nomoon_of_' + sys.argv[1] + '_timesteps.txt', 'w')
